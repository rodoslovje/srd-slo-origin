import { state, translations, groupRoots, formatAge, rawHaploData, rawPeopleData, getPersonTooltip, haploColors, eraColors } from "./shared.js";

export let ydnaInitialized = false;
let root, g, svg, zoom, tooltip;

export function initYDNA() {
    ydnaInitialized = true;

    svg = d3.select("#tree-container").append("svg").attr("width", "100%").attr("height", "100%");
    g = svg.append("g");
    zoom = d3.zoom().scaleExtent([0.05, 5]).on("zoom", (e) => g.attr("transform", e.transform));
    svg.call(zoom);
...
function renderYDNATree() {
    function buildHierarchy(nodes, leaves) {
        ...
        const rootNode = nodes.find((d) => d.parent === "") || { haplogroup: "A0000" };
    ...
    function pruneTree(node) {
        const isGroupRoot = Object.values(groupRoots).includes(node.haplogroup);
    ...
    let filteredPeople = rawPeopleData.filter(p => state.selectedGroups.has(p.group));
    ...
    const processedData = pruneTree(buildHierarchy(rawHaploData, filteredPeople));
    ...
    if (root) {
        ...
        root.each(d => oldNodes.set(d.data.id, d));
        newRoot.each(d => {
    ...
    groups.forEach((groupName) => {
        ...
        const target = newRoot.descendants().find((d) => d.data.haplogroup === groupRoots[groupName]);
    ...
    if (state.searchQuery) {
        zoomTargetNode = findPersonRecursively(root);
        if (zoomTargetNode) {
    ...
    if (!zoomTargetNode) {
        ...
        if (!state.selectedGroups.has(zoomTargetGroup)) zoomTargetGroup = state.selectedGroups.size > 0 ? Array.from(state.selectedGroups)[0] : null;
        if (zoomTargetGroup) {
            const findHg = (n, hg) => {
                if (n.data.haplogroup === hg) return n;
            ...
            zoomTargetNode = findHg(root, groupRoots[zoomTargetGroup]);
        }
    }
    ...
    if (zoomTargetNode) {
        zoomToNode(zoomTargetNode, 750);
    } else {
        zoomToNode(root, 750);
    }
}

function zoomToNode(d, duration = 750) {
    const scale = 0.75;
...
function update(source) {
    ...
    const nodeEnter = node.enter().append("g")
        .attr("class", (d) => {
            ...
            if (hasNote || Object.values(groupRoots).includes(d.data.haplogroup) || (d.data.isPerson && d.data.test && d.data.test.includes("Big Y"))) {
                cls += " node--prominent";
            }
            return cls;
        })
        ...
        .on("mouseover", (event, d) => {
            ...
            if (d.data.isPerson) {
                tooltip.html(getPersonTooltip(d.data, error));
            } else {
                let notePart = d.data.note && d.data.note.trim() !== "" && d.data.note !== translations[state.currentLang].notePathMissing ? ` (${d.data.note})` : "";
                if (!notePart) {
                    const rootGroupKey = Object.keys(groupRoots).find((k) => groupRoots[k] === d.data.haplogroup);
                    if (rootGroupKey) notePart = ` (${rootGroupKey})`;
                }
                tooltip.html(`${translations[state.currentLang].snpLabel}: <b>${d.data.haplogroup}${notePart}</b>${error}<br>${translations[state.currentLang].ageEstimate}: ${formatAge(d.data.age)}`);
            }
    ...
    nodeEnter.each(function (d) {
        ...
        if (d.data.isPerson) {
            ...
            const rootGroupKey = Object.keys(groupRoots).find((k) => groupRoots[k] === d.data.haplogroup);
            const color = d.data.isAutoPlaced
                ? "#e53e3e"
                : rootGroupKey ? (haploColors[rootGroupKey] || haploColors["default"])
                : "#cbd5e0";
